...
public class Npc extends Creature
{
	...
	public void onActionShift(Player player)
	{
		...
		if (player.isGM())
		{
			...
			html.setFile("data/html/admin/npcinfo.htm", player);
			...
			if (getSpawn() != null)
			{
				html.replace("%spawn%", getSpawn().getSpawnLocation().toString());
			...
			html.replace("%ai_ssinfo%", _currentSsCount + "[" + getTemplate().getSsCount() + "] - " + "%");
			html.replace("%ai_spsinfo%", _currentSpsCount + "[" + getTemplate().getSpsCount() + "] - " + "%");
			html.replace("%butt%", ((this instanceof Npc) ? "<button value=\"Shop\" action=\"bypass -h admin_show_shop " + getNpcId() + "\" width=65 height=19 back=\"L2UI_ch3.smallbutton2_over\" fore=\"L2UI_ch3.smallbutton2\">" : ""));
		...
		else if (Config.ALT_GAME_VIEWNPC)
		{
			...
			final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
			html.setFile("data/html/custom/dropNpc/npcinfo.htm", player);
			html.replace("%id%", getTemplate().getNpcId());
			html.replace("%lvl%", getTemplate().getLevel());
			html.replace("%name%", getTemplate().getName());
			html.replace("%loc%", getX() + " " + getY() + " " + getZ());
	...
	public void onBypassFeedback(Player player, String command)
	{
		if (command.equalsIgnoreCase("TerritoryStatus"))
		{
			...
			if (clan != null)
			{
				html.setFile("data/html/territorystatus.htm", player);
				html.replace("%clanName%", clan.getName());
				html.replace("%clanLeaderName%", clan.getLeaderName());
				html.replace("%taxPercent%", castle.getCurrentTaxPercent());
			}
			else
				html.setFile("data/html/territorynoclan.htm", player);
		...
		else if (command.startsWith("Link"))
		{
			...
			final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
			html.setFile("data/html/" + path, player);
			html.replace("%objectId%", getObjectId());
			player.sendPacket(html);
		}
		...
		else if (command.startsWith("observe_group"))
		{
			...
			final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
			html.setHtml(sb.toString(), player);
	...
	private static void showQuestWindowSingle(Player player, Npc npc, Quest quest)
	{
		if (quest == null)
		{
			...
			html.setHtml(Quest.getNoQuestMsg(), player);
			player.sendPacket(html);
		...
		if (quest.isRealQuest())
		{
			...
			if (player.getQuestList().getQuestState(quest.getName()) == null)
			{
				...
				{
					final NpcHtmlMessage html = new NpcHtmlMessage(npc.getObjectId());
					html.setHtml(Quest.getTooMuchQuestsMsg(), player);
					player.sendPacket(html);
	...
	public void showLotoWindow(Player player, int val)
	{
		...
		{
			html.setFile(getHtmlPath(npcId, 1), player);
		}
