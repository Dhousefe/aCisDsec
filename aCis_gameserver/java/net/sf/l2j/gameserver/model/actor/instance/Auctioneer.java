...
public final class Auctioneer extends Folk
{
	...
	public void onBypassFeedback(Player player, String command)
	{
			...
			try
			{
				...
				if (ch != null)
				{
					...
					if (auction != null)
					{
						...
						html.setFile("data/html/auction/AgitAuctionInfo.htm", player);
		...
		{
			final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
			html.setFile("data/html/auction/map_agit_" + RestartPointData.getInstance().getAgitMap(player) + ".htm", player);
		...
		else
		{
			...
			else if (actualCommand.equalsIgnoreCase("bid1"))
			{
				...
				try
				{
					...
					final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
					html.setFile("data/html/auction/AgitBid1.htm", player);
			...
			else if (actualCommand.equalsIgnoreCase("bidlist"))
			{
				try
				{
					...
					final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
					html.setFile("data/html/auction/AgitBidderList.htm", player);
			...
			else if (actualCommand.equalsIgnoreCase("cancelBid"))
			{
				try
				{
					...
					final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
					html.setFile("data/html/auction/AgitBidCancel.htm", player);
			...
			else if (actualCommand.equalsIgnoreCase("cancelAuction"))
			{
				...
				final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
				html.setFile("data/html/auction/AgitSaleCancel.htm", player);
			...
			else if (actualCommand.equalsIgnoreCase("sale"))
			{
				...
				final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
				html.setFile("data/html/auction/AgitSale1.htm", player);
				html.replace("%AGIT_DEPOSIT%", ch.getLease());
			...
			else if (actualCommand.equalsIgnoreCase("rebid"))
			{
				...
				final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
				html.setFile("data/html/auction/AgitBid2.htm", player);
			...
			else
			{
				...
				if (actualCommand.equalsIgnoreCase("auction"))
				{
					...
					try
					{
						...
						final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
						html.setFile("data/html/auction/AgitSale3.htm", player);
						html.replace("%x%", val);
						html.replace("%AGIT_AUCTION_END%", new SimpleDateFormat("dd-MM-yyyy HH:mm").format(auction.getEndDate()));
						html.replace("%AGIT_AUCTION_MINBID%", ch.getAuctionMin());
				...
				else if (actualCommand.equalsIgnoreCase("sale2"))
				{
					...
					final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
					html.setFile("data/html/auction/AgitSale2.htm", player);
	...
	{
		final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
		html.setFile("data/html/auction/auction.htm", player);
		html.replace("%objectId%", getObjectId());
	...
	private void showAuctionsList(String val, Player player)
	{
		...
		final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
		html.setFile("data/html/auction/AgitAuctionList.htm", player);
		html.replace("%AGIT_LIST%", sb.toString());
		html.replace("%AGIT_LINK_BACK%", "bypass -h npc_" + getObjectId() + "_start");
		player.sendPacket(html);
	}
	...
	private void showSelectedItems(Player player)
	{
		...
		if (!clan.hasClanHall() && clan.getAuctionBiddedAt() > 0)
		{
			...
			final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
			html.setFile("data/html/auction/AgitBidInfo.htm", player);
			html.replace("%AGIT_NAME%", ch.getName());
			html.replace("%AGIT_SIZE%", ch.getSize());
			html.replace("%AGIT_LEASE%", ch.getLease());
			html.replace("%AGIT_LOCATION%", ch.getTownName());
		...
		if (clan.hasClanHall())
		{
			...
			if (seller != null)
			{
				...
				html.setFile("data/html/auction/AgitSaleInfo.htm", player);
				html.replace("%AGIT_NAME%", ch.getName());
			...
			{
				html.setFile("data/html/auction/AgitInfo.htm", player);
				html.replace("%AGIT_NAME%", ch.getName());
