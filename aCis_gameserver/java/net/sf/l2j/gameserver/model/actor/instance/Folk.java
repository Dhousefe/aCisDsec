...
public class Folk extends Npc
{
	...
	public void showSkillList(Player player)
	{
		if (!getTemplate().canTeach(player.getClassId()))
		{
			...
			html.setFile("data/html/trainer/" + getTemplate().getNpcId() + "-noskills.htm", player);
	...
	public void showEnchantSkillList(Player player)
	{
		...
		{
			final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
			html.setHtml("<html><body> You must have 3rd class change quest completed.</body></html>", player);
			player.sendPacket(html);
			return;
		}
