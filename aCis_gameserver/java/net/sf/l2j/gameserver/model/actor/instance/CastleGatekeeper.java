...
public class CastleGatekeeper extends Folk
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		if (command.startsWith("tele"))
		{
			...
			html.setFile("data/html/castleteleporter/MassGK-1.htm", player);
	...
	public void showChatWindow(Player player)
	{
		...
		if (_teleportTask == null)
		{
			if (getCastle().getSiege().isInProgress() && getCastle().getAliveLifeTowerCount() == 0)
				html.setFile("data/html/castleteleporter/MassGK-2.htm", player);
			else
				html.setFile("data/html/castleteleporter/MassGK.htm", player);
		}
		else
		{
			html.setFile("data/html/castleteleporter/MassGK-1.htm", player);
