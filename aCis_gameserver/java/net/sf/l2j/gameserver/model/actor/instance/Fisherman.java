	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if (command.startsWith("FishingChampionship"))
		{
			if (!Config.ALLOW_FISH_CHAMPIONSHIP)
			{
				...
				html.setFile("data/html/fisherman/championship/no_fish_event001.htm", player);
		...
		else if (command.startsWith("FishingReward"))
		{
			...
			{
				final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
				html.setFile("data/html/fisherman/championship/no_fish_event_reward001.htm", player);
				player.sendPacket(html);
				return;
			}
