...
public class AutoFarmZone extends AutoFarmArea
{
	...
	public void onEnter(Creature character)
	{
		if (character.getActingPlayer() != null)
		{
			...
			if (!getProfile().isEnabled())
				character.sendMessage("Voce entrou em uma zona de AutoFarm.");
		}
		else
	...
	public void onExit(Creature character)
	{
		if (character.getActingPlayer() != null && !getProfile().isEnabled())
			character.sendMessage("Voce saiu da zona de AutoFarm.");
	...
	public Location findValidLocation()
	{
		...
		int attempt = 0;
		int radius = 100; // distancia inicial para afastar
		...
		while (attempt < 10)
		{
			...
			{
				return center; // Encontrou um local valido
			}

			// Aumenta a distancia do centro e tenta novamente
			double angle = Math.random() * 2 * Math.PI; // Gera um angulo aleatorio
			...
			radius += 100; // Aumenta o raio para a proxima tentativa
			attempt++;
		}

		return null; // Se nao encontrar um local valido apos 10 tentativas
	}
	...
	{
		// Enquanto o player estiver no caminho ate o centro ou o monstro, ele entrara na zona
		...
		// Localizacao aleatoria dentro da zona
		for (int i = 0; i < 10; i++)
		{
		...
		// O player tere que se locomover manualmente
		return null;
	}
