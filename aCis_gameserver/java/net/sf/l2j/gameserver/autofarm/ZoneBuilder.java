...
public class ZoneBuilder
{
	...
	public void removePoint(Player player, int nodeId)
	{
		...
		// Criar novos pontos importante para atualizar o preview, nomes e ID
	...
	private static void addPoint(ExServerPrimitive debug, Location loc)
	{
		...
		{
			final Map<Integer, Point> points = debug.getPoints();
			final Point previousPoint = points.get(points.keySet().stream().max(Integer::compareTo).orElse(null));
	...
	/*
	 * Usamos um pacote diferente porque esse sera enviado constatemente
	 */
	...
	private static void previewCylinder(Player player, ExServerPrimitive debug, int radius)
	{
		...
		for (int i = 1; i <= count; i++)
		{
			...
			// Atualiza o ponto anterior para a proxima linha
			prevX = x;
			prevY = y;
		}
	}
	...
	/*
	 * O zoneId  enviado porque ele pode ainda nao ter sido setado
	 */
