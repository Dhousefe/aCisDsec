...
public class AutoFarmRoutine
{
	...
	public synchronized void start()
	{
		...
		{
			stop("Sistema nao pode ser utilizado agora");
			return;
		}
		...
		{
			stop("Seu character nao consegue atacar mais");
			return;
		}
		
		if (_fp.getEndTime() != 0 && _fp.getFinalEndTime() < System.currentTimeMillis())
		{
			// Se ainda estiver atacando, sera encerrado assim que o monstro for morto
			if (!isPlayerAttacking(player))
				stop("Duracao programada foi concluida");
	...
	private synchronized void run(Player player)
	{
		...
		// Alta prioridade porque muitas acoes dependem deles
		...
		// Antes de comecar a atacar, nos nos preparamos
		...
		// Nao esta dentro da zona. Possiveis cenarios:
		// Character pode estar no caminho para a area
		// Esta atacando um monstro pelo lado de fora da area
		// O player forcou o movimento do character
		if (area.getType() == AutoFarmType.ZONA && !isPlayerAttacking(player))
		{
			// Se saiu para atacar nos permitimos, pode ter feito por ser o caminho mais curto
			// Mas se saiu e nao tem a intencao de atacar, entao deve voltar
			// Nao e interessante sempre forcar a volta porque o novo destino pode ser algum nao interessante, portanto dependeria de mais analises
		...
		// Se nao pegarmos a erva antes, ela vai desaparecer
		...
		if (!isPlayerAttacking(player) && (player.getAI().getCurrentIntention().getType() != IntentionType.ATTACK || !GeoEngine.getInstance().canMoveAround(player.getX(), player.getY(), player.getZ())))
		{
			...
			{
				// Target esta em uma distancia compativel, e visivel e o player consegue movimentar-se na sua direcao
