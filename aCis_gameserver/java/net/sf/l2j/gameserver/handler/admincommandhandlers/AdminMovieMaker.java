...
public class AdminMovieMaker implements IAdminCommandHandler
{
			...
			switch (param)
			{
				case "add":
					if (!st.hasMoreTokens())
					{
						...
						html.setFile("data/html/admin/movie/add_sequence.htm", player);
					...
					try
					{
						...
						if (!st.hasMoreTokens())
						{
							...
							final NpcHtmlMessage html = new NpcHtmlMessage(0);
							html.setFile("data/html/admin/movie/edit_sequence.htm", player);
	...
	private static void mainHtm(Player player)
	{
		...
		if (_sequences.isEmpty())
			html.setFile("data/html/admin/movie/main_empty.htm", player);
		else
		{
			...
			html.setFile("data/html/admin/movie/main_notempty.htm", player);
