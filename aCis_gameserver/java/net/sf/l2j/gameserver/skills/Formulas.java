...
public final class Formulas
{
	...
	public static double calcBlowDamage(Creature attacker, Creature target, L2Skill skill, ShieldDefense sDef, boolean ss)
	{
		...
		final double pvpMul;
		if (isPvP) {
			pvpMul = ClassBalanceManager.getInstance().getValueFor(ClassBalanceType.BLOW, attacker, target);
		} else {
			pvpMul = 1.;
		}
	...
	public static final double calcPhysicalAttackDamage(Creature attacker, Creature target, ShieldDefense sDef, boolean crit, boolean ss)
	{
		...
		if (attacker instanceof Playable && target instanceof Playable) {
			if (crit) {
				...
				pvpMul = ClassBalanceManager.getInstance().getValueFor(ClassBalanceType.CRITICAL, attacker, target);
				}
			 
			else {
				
				pvpMul = ClassBalanceManager.getInstance().getValueFor(ClassBalanceType.NORMAL, attacker, target);
				}
	...
	public static final double calcPhysicalSkillDamage(Creature attacker, Creature target, L2Skill skill, ShieldDefense sDef, boolean crit, boolean ss)
	{
		...
		if (attacker instanceof Playable && target instanceof Playable) {
			if (crit) {
				if (skill != null && skill.getSkillType() == SkillType.PDAM) {
					pvpMul = ClassBalanceManager.getInstance().getValueFor(ClassBalanceType.PHYSICAL_SKILL_CRITICAL, attacker, target);
				} else {
				...
				if (skill != null && skill.getSkillType() == SkillType.PDAM) {
					pvpMul = ClassBalanceManager.getInstance().getValueFor(ClassBalanceType.PHYSICAL_SKILL_DAMAGE, attacker, target);
				} else {
					pvpMul = ClassBalanceManager.getInstance().getValueFor(ClassBalanceType.NORMAL, attacker, target);
				}
			}
		...
		// Calcula o multiplicador de dano aleatório para habilidades físicas (exceto CHARGEDAM).
		if (skill.getEffectType() != SkillType.CHARGEDAM) {
			// Verifica se o atacante e o alvo são jogadores (PvP).
			//if (skill.getEffectType() != SkillType.CHARGEDAM) {
    		rndMul = attacker.getRandomDamageMultiplier();
			//}
			//if (attacker instanceof Player && target instanceof Player) {
				// Multiplica o dano aleatório pelo valor de balanceamento de classes para o tipo BLOW.
				//double randomMultiplier = attacker.getRandomDamageMultiplier();
				//double classBalanceMultiplier = ClassBalanceManager.getInstance().getValueFor(ClassBalanceType.BLOW, attacker, target);
				//rndMul = randomMultiplier * classBalanceMultiplier;
				//rndMul = classBalanceMultiplier;
			//}
		}
		// Race multiplier.
		if (target instanceof Npc targetNpc)
	...
	public static final double calcMagicDam(Creature attacker, Creature target, L2Skill skill, ShieldDefense sDef, boolean ss, boolean bss, boolean mcrit)
	{
		...
		if (Config.MAGIC_FAILURES && !calcMagicSuccess(attacker, target, skill))
		{
			...
			if (attacker instanceof Playable && target instanceof Playable) {
				// Aplica o valor de balanceamento para M_CRITICAL em cenários PvP.
				damage *= ClassBalanceManager.getInstance().getValueFor(ClassBalanceType.M_CRITICAL, attacker, target);
			} else {
				// Aplica o multiplicador padrão para M_CRITICAL em cenários não PvP.
				damage *= 4;
			}
		}
		// Pvp bonuses for dmg
