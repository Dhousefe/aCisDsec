...
public final class ClassMaster extends Folk
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if (command.startsWith("change_class"))
		{
			...
			if (checkAndChangeClass(player, val))
			{
				...
				html.setFile(getHtmlPath(getNpcId(), 4), player);
		...
		else if (command.startsWith("become_noble"))
		{
			...
			if (player.isNoble())
				html.setFile(getHtmlPath(getNpcId(), 5), player);
			else
			{
				html.setFile(getHtmlPath(getNpcId(), 6), player);
		...
		else
		{
			...
			else
			{
				...
				if (player.getStatus().getLevel() >= minLevel || Config.ALLOW_ENTIRE_TREE)
				{
					...
					{
						html.setFile(getHtmlPath(getNpcId(), 3), player);
	...
	private static final boolean checkAndChangeClass(Player player, int val) {
		...
		// Verifica se o jogador possui todos os itens necessários para a mudança de classe
		...
		// Recompensa o jogador com itens
		...
		// Após a mudança de classe, equipa os itens com base na classe
