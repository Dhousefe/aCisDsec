...
public class ClanHallDoorman extends Doorman
{
	...
	public void showChatWindow(Player player)
	{
		...
		{
			if (canProvideWyvernService())
				html.setFile("data/html/clanHallDoormen/doormen_wyvern.htm", player);
			else
				html.setFile("data/html/clanHallDoormen/doormen.htm", player);
		...
		else
		{
			...
			{
				html.setFile("data/html/clanHallDoormen/doormen-no.htm", player);
				html.replace("%leadername%", owner.getLeaderName());
				html.replace("%clanname%", owner.getName());
			}
			else
			{
				html.setFile("data/html/clanHallDoormen/emptyowner.htm", player);
	...
	protected final void openDoors(Player player, String command)
	{
		...
		final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
		html.setFile("data/html/clanHallDoormen/doormen-opened.htm", player);
		html.replace("%objectId%", getObjectId());
		player.sendPacket(html);
	}
	...
	protected final void closeDoors(Player player, String command)
	{
		...
		final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
		html.setFile("data/html/clanHallDoormen/doormen-closed.htm", player);
		html.replace("%objectId%", getObjectId());
		player.sendPacket(html);
	}
	...
	{
		final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
		html.setFile("data/html/clanHallDoormen/wyvernmanager-" + val + ".htm", player);
		html.replace("%objectId%", getObjectId());
