...
import net.sf.l2j.gameserver.util.FuriousMobMod;
...
public class Monster extends Attackable
{
	...
	private int _titleColor = 0xFFFFFF; // Default white color
	private boolean _furious = false;
	...
	/**
	 * Sets the furious state of the monster.
	 * @param furious true if the monster is furious, false otherwise
	 */
	...
	/**
	 * Returns whether the monster is furious.
	 * @return true if furious, false otherwise
	 */
	...
	/**
	 * Sets the title color of the monster.
	 * @param color the RGB color as int
	 */
	...
	/**
	 * Gets the title color of the monster.
	...
	@Override
	protected void onCustomDeath(Creature killer) {
		if (isFurious() && killer != null && killer.getActingPlayer() != null) {
			Player player = killer.getActingPlayer();
			player.addItem(FuriousMobMod.DROP_ITEM_ID, FuriousMobMod.DROP_AMOUNT, true);
			String msg = "Parabéns! " + player.getName() + " derrotou um mob " + getName() + " e recebeu uma recompensa especial!";
			broadcastPacket(new net.sf.l2j.gameserver.network.serverpackets.CreatureSay(
				player.getObjectId(),
				net.sf.l2j.gameserver.enums.SayType.TELL,
				"Mob Furioso",
				msg
			));
			broadcastPacket(new net.sf.l2j.gameserver.network.serverpackets.PlaySound("ItemSound.quest_finish"));
			broadcastPacket(new net.sf.l2j.gameserver.network.serverpackets.MagicSkillUse(player, player, 2025, 1, 1000, 0));
		}
	}

	private double _furiousHpMult = 1.0;
	private double _furiousAtkMult = 1.0;

	/**
	 * Seta o HP atual do monstro.
	...
	public int getMaxHp()
	{
		return (int) (getStatus().getMaxHp() * _furiousHpMult);
	}

	public int getPAtk(Creature target)
	{
		return (int) (getStatus().getPAtk(target) * _furiousAtkMult);
	}

	public int getMAtk(Creature target, L2Skill skill)
	{
		return (int) (getStatus().getMAtk(target, skill) * _furiousAtkMult);
	}

	public void updateNameAndTitle(Player player) {
		Player knownPlayer = getKnownType(Player.class).stream().findFirst().orElse(null);
		player = knownPlayer; // Se player for nulo, usa o primeiro conhecido
    if (player == null) {
		
        // Atualiza para todos os jogadores que conhecem este monstro
        forEachKnownType(Player.class, this::sendInfo);
		broadcastPacket(new NpcInfo(this, null));
		knownPlayer.sendPacket(new NpcInfo(this, null));
		
		        
    } else {
        // Atualiza apenas para o player informado
        sendInfo(player);
        player.broadcastTitleInfo();
		broadcastPacket(new NpcInfo(this, knownPlayer));
		player.sendPacket(new NpcInfo(this, knownPlayer));
		//knownPlayer.sendPacket(new NpcInfo(this, knownPlayer));
    }
	}
	
	// Store the original name of the monster
	private String _originalName;

	public void onCustomSpawn(Player player) {
    // Faça o que quiser com o player e o monstro
    updateNameAndTitle(player);
	}
	...
	public void onSpawn()
	{
		...
		// Verifica se existe algum player a até 2000 de distância
		boolean hasNearbyPlayer = getKnownType(Player.class).stream()
			.anyMatch(p -> p != null && MathUtil.checkIfInRange(2000, this, p, true));

		if (FuriousMobMod.ENABLED && Math.random() < FuriousMobMod.CHANCE) {
			setTitle(FuriousMobMod.TITLE);
			setName("Furioso " + _originalName);
			if (hasNearbyPlayer)
				updateNameAndTitle(getKnownType(Player.class).stream().findFirst().orElse(null));
			setTitleColor(FuriousMobMod.TITLE_COLOR);
			setFurious(true);
			_furiousHpMult = FuriousMobMod.HP_MULT;
			_furiousAtkMult = FuriousMobMod.ATK_MULT;
			setHp(getMaxHp());
			broadcastPacket(new net.sf.l2j.gameserver.network.serverpackets.MagicSkillUse(this, this, 1034, 1, 1000, 0));
		} else {
			setFurious(false);
			_furiousHpMult = 1.0;
			_furiousAtkMult = 1.0;
			if (_originalName != null)
				setName(_originalName);
			if (hasNearbyPlayer)
				updateNameAndTitle(getKnownType(Player.class).stream().findFirst().orElse(null));
		}
		super.onSpawn();
