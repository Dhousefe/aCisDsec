...
public class PlayerBuffer extends Folk
{
   ...
   private void showMainWindow(Player activeChar)
   {
       ...
       html.setFile(getHtmlPath(getNpcId(), 0), null);
   ...
   public void onBypassFeedback(Player player, String command)
   {
       ...
       if (player.isDead()) {
        ...
        // Nova restrição: verificar se o jogador está registrado nas Olimpíadas
        if (OlympiadManager.getInstance().isRegistered(player)) {
            player.sendMessage("You can't use buffer while registered in the Olympiad.");
            return;
        }
       ...
       if (!st.hasMoreTokens()) {
           LOGGER.warn("Received an empty or invalid command: {}", command);
           return;
       }
       ...
       if (currentCommand.startsWith("createscheme")) {
           try {
               if (!st.hasMoreTokens()) {
                   player.sendMessage("Invalid command format. Scheme name is missing.");
                   return;
               }

               final String schemeName = st.nextToken();
               LOGGER.info("Processing createscheme command: PlayerID={}, SchemeName={}", player.getObjectId(), schemeName);
               ...
               LOGGER.info("Scheme created successfully: PlayerID={}, SchemeName={}", player.getObjectId(), schemeName);

               showGiveBuffsWindow(player);
           } catch (Exception e) {
               LOGGER.error("Error processing createscheme command for PlayerID={}: {}", player.getObjectId(), e.getMessage(), e);
               player.sendMessage("An error occurred while creating the scheme.");
           }
       }
       else if (currentCommand.startsWith("bufflist"))
       {
       ...
       else if (currentCommand.startsWith("openlist"))
       {
           ...
           {
               html.setFile("data/html/mods/buffer/" + htmfile + ".htm", player);
               ...
               html.setFile("data/html/mods/buffer/" + category + "/" + htmfile + ".htm", player);
       ...
       else if (currentCommand.startsWith("dobuff"))
       {
           ...
           NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
           html.setFile("data/html/mods/buffer/" + category + "/" + windowhtml + ".htm", player);
           html.replace("%objectId%", String.valueOf(getObjectId()));
           html.replace("%name%", player.getName());
           player.sendPacket(html);
       }
       ...
       else if (currentCommand.startsWith("skill"))
       {
           ...
           // Define schemeName before using it
           //final String schemeName = st.nextToken();
           ...
           if (currentCommand.startsWith("skillselect") && !schemeName.equalsIgnoreCase("none"))
           {
               if (skills.contains(skillId)) {
                   player.sendMessage("This skill is already in the scheme.");
               } else {
                   if (skills.size() < player.getMaxBuffCount()) {
                       skills.add(skillId);
                       player.sendMessage("Buff added to the scheme.");

                       // Salva o esquema no banco de dados
                       BufferManager.getInstance().setScheme(player.getObjectId(), schemeName, new ArrayList<>(skills));
               ...
               try {
                   ...
                   //final String groupType = st.nextToken();
                   //final String schemeName = st.nextToken();
                   //final int skillIdToRemove = Integer.parseInt(st.nextToken());
                   //final int page = Integer.parseInt(st.nextToken());

                   // Obtém o esquema do jogador
                   //final List<Integer> skills = BufferManager.getInstance().getScheme(player.getObjectId(), schemeName);

                   if (skills != null && skills.contains(skillId)) {
                       // Remove a skill do mapa em memória
                       skills.remove(Integer.valueOf(skillId));

                       // Atualiza o esquema no banco de dados
                       BufferManager.getInstance().updateScheme(player.getObjectId(), schemeName, skills);

                       player.sendMessage("Skill removed from the scheme.");
                   } else {
                       player.sendMessage("The skill is not in the scheme.");
                   }

                   // Atualiza a página
                   showEditSchemeWindow(player, groupType, schemeName, page);
               } catch (Exception e) {
                   LOGGER.error("Error processing skillunselect command for PlayerID={}: {}", player.getObjectId(), e.getMessage(), e);
                   player.sendMessage("An error occurred while removing the skill from the scheme.");
               }
           }
       }
       else if (currentCommand.startsWith("skillselect")) {
           final String groupType = st.nextToken();
           final String schemeName = st.nextToken();
           final int skillId = Integer.parseInt(st.nextToken());
           ...
           if (skills.contains(skillId)) {
               ...
               if (skills.size() < player.getMaxBuffCount()) {
                   ...
                   // Atualiza a página
                   showEditSchemeWindow(player, groupType, schemeName, 1);
               } else {
                   player.sendMessage("This scheme has reached the maximum amount of buffs.");
               }
           }
       ...
       else if (currentCommand.startsWith("deletescheme"))
       {
           try
           {
               ...
               if (schemes != null && schemes.containsKey(schemeName))
                   // Chama a função deleteScheme para remover o esquema
                   BufferManager.getInstance().deleteScheme(player.getObjectId(), schemeName);
                   if (schemes != null) {
                       schemes.remove(schemeName);
                   }
                   player.sendMessage("Scheme deleted successfully.");
           }
           catch (Exception e) {
            LOGGER.error("Error processing deletescheme command for PlayerID={}: {}", player.getObjectId(), e.getMessage(), e);
            player.sendMessage("An error occurred while deleting the scheme.");
            }
           showGiveBuffsWindow(player);
       }
   ...
   private void showGiveBuffsWindow(Player player)
   {
       ...
       final NpcHtmlMessage html = new NpcHtmlMessage(0);
       html.setFile(getHtmlPath(getNpcId(), 1), player);
   ...
   private void showEditSchemeWindow(Player player, String groupType, String schemeName, int page)
   {
       ...
       html.setFile(getHtmlPath(getNpcId(), 2), player);
