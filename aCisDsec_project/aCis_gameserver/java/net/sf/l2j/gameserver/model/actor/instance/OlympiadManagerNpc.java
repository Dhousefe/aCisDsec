...
public class OlympiadManagerNpc extends Folk
{
	...
	public void showChatWindow(Player player, int val)
	{
		...
		html.setFile("data/html/olympiad/" + filename, player);
	...
	public void onBypassFeedback(Player player, String command)
	{
		if (command.startsWith("OlympiadNoble"))
		{
			...
			{
				html.setFile(Olympiad.OLYMPIAD_HTML_PATH + "noble_cant_cw.htm", player);
				player.sendPacket(html);
				return;
			}
			...
			{
				html.setFile(Olympiad.OLYMPIAD_HTML_PATH + "noble_cant_sub.htm", player);
				html.replace("%objectId%", getObjectId());
				player.sendPacket(html);
				return;
			}
			...
			{
				html.setFile(Olympiad.OLYMPIAD_HTML_PATH + "noble_cant_thirdclass.htm", player);
				html.replace("%objectId%", getObjectId());
				player.sendPacket(html);
				return;
			}
			...
			switch (val)
			{
				...
				case 2: // Show waiting list
					...
					html.setFile(Olympiad.OLYMPIAD_HTML_PATH + "noble_registered.htm", player);
				...
				case 3: // There are %points% Grand Olympiad points granted for this event.
					...
					html.setFile(Olympiad.OLYMPIAD_HTML_PATH + "noble_points1.htm", player);
				...
				case 6: // request tokens reward
					...
					if (points >= 50 || player.isHero() || HeroManager.getInstance().isInactiveHero(player.getObjectId()))
						html.setFile(Olympiad.OLYMPIAD_HTML_PATH + "noble_settle.htm", player);
					else
						html.setFile(Olympiad.OLYMPIAD_HTML_PATH + "noble_nopoints2.htm", player);
			...
			switch (val)
			{
				case 2: // Show rank for a specific class, example >> Olympiad 1_88
					...
					if (classId >= 88 && classId <= 118)
					{
						...
						html.setFile(Olympiad.OLYMPIAD_HTML_PATH + "noble_ranking.htm", player);
				...
				case 3: // Spectator overview
					html.setFile(Olympiad.OLYMPIAD_HTML_PATH + "olympiad_observe_list.htm", player);
				...
				case 5: // Hero pending state.
					if (HeroManager.getInstance().isInactiveHero(player.getObjectId()))
					{
						html.setFile(Olympiad.OLYMPIAD_HTML_PATH + "hero_confirm.htm", player);
						html.replace("%objectId%", getObjectId());
						player.sendPacket(html);
					}
					break;
				...
				case 7: // Main panel
					html.setFile(Olympiad.OLYMPIAD_HTML_PATH + "hero_main.htm", player);
