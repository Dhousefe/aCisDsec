...
public class AutoFarmRoute extends AutoFarmArea
{
	...
	{
		// Aumentamos o range para obter mais opcoes, mas serao retornados apenas os comataveis
		if (getProfile().getFinalRadius() < 100)
		{
			...
			for (Monster m :getOwner().getKnownTypeInRadius(Monster.class, getProfile().getFinalRadius() * 2))
			{
				...
				// Monstros que estao fora do range de ataque so sao retornados caso ele esteja tentando atacar o player
	...
	/**
	 * @return A rota ainda nao foi iniciada
	 */
	...
	/**
	 * @return O player esta comecando agora a rota (ainda nao chegou no index mais proximo)
	 */
	...
	public void moveToNextPoint()
	{
		...
		{
			// Player esta com problemas para se movimentar
			...
			// O player acabou de chegar no primeiro node, podemos liberar as outras areas da routine
